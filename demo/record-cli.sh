...
# ─────────────────────────────────────────────────────────────────────────────
# Check node status (fresh node) — human-readable is the default
...
run "$BIN" upload "$FIXTURES/sample1.mp4"
...
run "$BIN" upload-folder "$FIXTURES/folder-demo"
...
run "$BIN" download "$CID" "$DOWNLOAD_DIR"

echo "─────────────────────────────────────"
sleep 5

# ─────────────────────────────────────────────────────────────────────────────
# Show JSON output explicitly with --json flag
echo -e "\n\033[90m# 7. Same status command with --json (machine-readable)\033[0m"
run "$BIN" status --json
...
echo -e "\n\033[90m# 8. Clear cached (non-user-owned) videos\033[0m"
run "$BIN" cache clear
