...
Outputs: demo/gui-map.svg  demo/gui-upload.svg
"""
...
def map_screen(slider_pct=0.60):
    ...
    # Date label
    # Approximate: slider_pct=0 → 2026-02-12, 1 → 2026-03-14 23:00
...
def upload_screen():
    ...
    items = [
        ("event-footage-001.mp4",  100, "done",      GREEN),
        ("protest-march-clip2.mp4",100, "done",      GREEN),
        ("folder-clip-001.mp4",    100, "done",      GREEN),
        ("folder-clip-002.mp4",    100, "done",      GREEN),
        ("event-footage-001.mp4",  100, "duplicate", ORANGE),
    ]
    item_h = 52
    for fname, pct, status, sc in items:
        out.append(rect(24, y, W - 48, item_h - 6, DARK2, rx=5))
        # icon
        icon = "⚠️" if status == "duplicate" else ("✅" if status == "done" else "⏳")
        out.append(text(38, y + item_h//2 + 3, icon, size=14, anchor="middle"))
        # filename
        out.append(text(62, y + item_h//2 - 4, fname, fill=TEXT, size=12,
                        family="monospace"))
        # status badge
        out.append(rect(W - 160, y + 10, 110, 22, sc, rx=11))
        out.append(text(W - 160 + 55, y + 25, status.upper(),
...
def compose(screen_fn, active_tab, screen_name):
    parts = [
...
svg_map = compose(lambda: map_screen(slider_pct=0.60), active_tab=0, screen_name="Map")
with open(os.path.join(out_dir, "gui-map.svg"), "w") as f:
    f.write(svg_map)

# Upload tab — active (index 1)
svg_upload = compose(upload_screen, active_tab=1, screen_name="Upload")
with open(os.path.join(out_dir, "gui-upload.svg"), "w") as f:
    f.write(svg_upload)

print(f"Generated gui-map.svg and gui-upload.svg  ({W}×{H})")
